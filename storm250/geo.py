...
_GRID_PLAN_CACHE_MAX = 16 # in practice, since we are only converting one sweep (the composite) to a metric grid,
...
def _get_or_build_grid_plan(comp_scan, class_field, grid_res_m, pad_m, debug=False):
    ...
    valid_ray_idx = ray_idx_2d[valid_geo_2d].astype(np.int32, copy=False)   # flat list of ray indices  --- a specific gate's ray + gate location (range) can be found given one index 
    ...
    for y0 in range(0, ny, block_rows):
        ...
        _, idx = tree.query(np.column_stack((Xb, Yb)), k=1, workers=-1) # -> for each grid cell, which index in the tree is the closest
